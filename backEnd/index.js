...
app.post('/signin', async (req, res) =>{
  ...
  if(!user){
    ...
    if(!isPasswordValid){
      ...
      res.json({token: token, message: "success", id: user._id, isOnBoarded: user.isOnBoarded, role: user.role});
    }
  }
})
...
app.post('/google', async(req, res) =>{
  ...
  if(!user){
    ...
    const token = newUser.generateAuthToken();
    res.json({message: "success", id: newUser._id, isOnBoarded: newUser.isOnBoarded, token: token, role: newUser.role});
  }
  else{
    const token = user.generateAuthToken();
    res.json({message: "success", id: user._id, isOnBoarded: user.isOnBoarded, token: token, role: user.role});
  }
})
...
app.post('/role', async (req, res)=>{
  const arr = Array.isArray(req.body.selected_skills) ? req.body.selected_skills.map(s=>s.value) : [];
...
app.post('/projects', async (req, res) => {
  const { id, title, descrption, tags, contributorRequired, expectedDuration, guidelines } = req.body;
...
app.post('/application', upload.single('resume'), async (req, res) =>{
  const {cover, proof, time, projectId, projectName, creatorId, userId} = req.body;
  ...
  if(!projectId || !userId){
    ...
    const application = new Application({
      cover, proof, time, resume: resumePath, projectId, projectName, creatorId, userId
    });
...
app.post('/get-contributors', async (req, res) => {
  ...
  res.json({message: "success", contributors: contributors, projects: projects});
});
  ...
  try {
    ...
    const filteredProjects = projects.map(project => {
      const assignedTasks = project.tasks.filter(task =>
        task.assignedTo.toString() === contributorId
      );
      return {
        _id: project._id,
        title: project.title,
...
app.post('/update', async (req, res) => {
  try {
    const application = await Application.findByIdAndUpdate(req.body.id, {
