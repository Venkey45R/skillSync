...
function ReceivedApplications() {
  ...
  useEffect(()=>{
    axios.post('http://localhost:3000/all-application', {userId}).then((res)=>{  
  ...
  const update = (id, status) => {
    axios.post('http://localhost:3000/update', { id, status }).then((res) => {
          ...
          {application.length === 0 ? (
            ...
            application.map((application, index) => (
              ...
              >
                ...
                <div className='mb-4'>
                    {application.proof !== "" && <div className='flex gap-4'>Attached proof of work: <a href={`http://${application.proof}`} target='_blank' rel='noopener noreferrer' className='text-blue-500 underline'>Click here</a></div> }
                </div>
                <div className='mb-4'>
                ...
                {application.status === "pending" ?
                  <div className="flex justify-between gap-6">
                    <button className='w-full py-2 text-center text-white bg-green-500 rounded-xl' onClick={()=>update(application._id, "accept")}>Accept</button>
                    <button className='w-full py-2 text-center text-white bg-red-500 rounded-xl' onClick={()=>update(application._id, "reject")}>Reject</button>
                  </div>
                  :
