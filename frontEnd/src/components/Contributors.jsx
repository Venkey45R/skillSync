...
function Contributors() {
  ...
  useEffect(() => {
    axios.post('http://localhost:3000/get-contributors', { id }).then((res) => {
  ...
  const handleSubmit = async () =>{
    axios.post('http://localhost:3000/task', {...taskDetails, projectId: projectId, contributorId: contributorId}).then((res) =>{
      ...
      {popup && (
        <div className='fixed inset-0 z-50 flex items-center justify-center text-white border border-black bg-black/50'>
          <div className="w-full max-w-md p-6 bg-white border border-black shadow-lg rounded-xl">
            <h3 className="mb-6 text-xl font-semibold text-center text-black">Assign Task</h3>
            <input
              type="text"
              name="title"
            ...
            <div className="flex justify-end space-x-2">
              <button onClick={() => setPopup(false)} className="px-4 py-2 text-white bg-gray-500 rounded">
                Cancel
              </button>
        ...
        {projects.length > 0 ? (
          <div className='mx-10 mt-10 space-y-12'>
            {projects.map((project) => (
              <div key={project._id}>
                <h2 className='mb-4 text-2xl font-bold text-blue-400'>Project: {project.title}</h2>
                ...
                <div className='grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3'>
                  {project.contributors.map(contributorId => {
                    ...
                    const tasks = project.tasks.filter(task => task.assignedTo === contributorId);

                    return (
                      <div key={contributor._id} className='p-4 transition-all border border-white shadow-lg bg-white/10 rounded-2xl min-w-60 backdrop-blur-lg hover:bg-white/20'>
                        <p className='text-lg font-bold text-white'>{contributor.firstName}</p>
                        <p className='mb-2 text-sm text-gray-300'>{contributor.email}</p>
                        ...
                        <div className='justify-between block my-2 lg:flex'>
                            <button className='py-2 my-2 mt-auto text-white bg-green-500 cursor-pointer min-w-60 lg:min-w-52 rounded-2xl' onClick={() => assignTask(project._id, contributor._id)}>Assign Task</button><br />
                            <button className='py-2 my-2 mt-auto text-white bg-blue-500 cursor-pointer min-w-60 lg:min-w-52 rounded-2xl'>Update Status</button><br />
                        </div>
                      </div>
                    );
                  })}
