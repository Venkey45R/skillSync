...
function ProjectListing() {
  ...
  useEffect(() => {
    if (id) {
      setLoading(true);
      axios.post(`${API_BASE}/get-allprojects`, {}).then((res) => {
  ...
  const applyFilters = () => {
    ...
    if (filters.tech.length > 0) {
      temp = temp.filter(
        (p) =>
          p.tags &&
          filters.tech.some((stack) => p.tags.includes(stack.toLowerCase()))
      );
    }
      ...
      <div className="px-6">
        ...
        {isFilters && (
          <div className="flex flex-col w-full gap-3 px-10 py-5 mx-auto font-bold max-w-7xl bg-white/10 rounded-xl">
            <h3 className="text-xl font-semibold text-[#00ffc3]">
              Project Status
            </h3>
            ...
            <h3 className="text-xl font-semibold text-[#00ffc3] mt-4">
              Tech Stack
            </h3>
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
              {[
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("duration", "< 2 weeks")}
                />{" "}
                Less than 2 weeks
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("duration", "< 6 weeks")}
                />{" "}
                Less than 6 weeks
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("duration", "> 8 weeks")}
                />{" "}
                More than 8 weeks
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <label>
                <input
                  type="checkbox"
                  onChange={() =>
                    handleFilterChange("contributorsRequired", "< 4")
                  }
                />{" "}
                Less than 4
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() =>
                    handleFilterChange("contributorsRequired", "< 8")
                  }
                />{" "}
                Less than 8
              </label>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("applications", "< 5")}
                />{" "}
                Less than 5
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("applications", "< 10")}
                />{" "}
                Less than 10
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("applications", "> 25")}
                />{" "}
                More than 25
              </label>
            </div>
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
              <label>
                <input
                  type="checkbox"
                  onChange={() =>
                    handleFilterChange("postedIn", "Last 24 hours")
                  }
                />{" "}
                Last 24 hours
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("postedIn", "Last 3 days")}
                />{" "}
                Last 3 days
              </label>
              <label>
                <input
                  type="checkbox"
                  onChange={() => handleFilterChange("postedIn", "Last 7 days")}
                />{" "}
                Last 7 days
              </label>
            </div>
            ...
            <div className="flex justify-center mt-6">
              <button
                className="px-6 py-2 rounded-xl bg-[#00ffc3]/20 border border-[#00ffc3] cursor-pointer"
          ...
          {filteredProjects.length === 0 ? (
            ...
            filteredProjects.map((project, index) => (
              ...
              >
                ...
                <button
                  className="w-full py-2 mt-auto text-white bg-blue-500 cursor-pointer rounded-2xl"
