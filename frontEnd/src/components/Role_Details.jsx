  ...
  const handleSubmit = () =>{
    if(selected_skills.length > 5){
      ...
      if(!id){
        ...
        axios.post('http://localhost:3000/role', {id, selected_skills, projects, experience}).then((res)=>{
          if(res.data === "success"){
      ...
      <div className='flex items-center justify-center p-5'>
        ...
        <div className="mb-3">
          ...
          <div className="rounded-xl">
            <Select options={skills} value={selected_skills} onChange={handleChange} isMulti={true} />
          </div>
        </div>
        <div className="mb-3">
          <label className="block mb-3 text-sm font-medium text-white">
          ...
          <textarea rows={3} placeholder='explain about the technologies used and keep it simple...' onChange={(e) => setProjects(e.target.value)} className='w-full px-4 py-3 text-white rounded-xl bg-white/20 placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-sky-400 backdrop-blur-sm' />
        </div>
        <div className="mb-3">
          <label className="block mb-2 text-sm font-medium text-white">
            Experience <span className="text-sm text-white/60">(in months)</span>
          </label>
          <input type="text" placeholder="Enter your experience" onChange={(e) => setExperience(e.target.value)} className="w-full px-4 py-3 text-white rounded-xl bg-white/20 placeholder-white/60 focus:outline-none focus:ring-2 focus:ring-sky-400 backdrop-blur-sm" />
        </div>
